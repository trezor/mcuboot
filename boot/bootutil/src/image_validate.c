...
_Static_assert(IMAGE_TLV_PQ_IMAGE_HASH == IMAGE_TLV_SHA256,
               "image_pq.h image-hash TLV drifted from bootutil/image.h");
                      ...
                      int seed_len, uint8_t *out_hash)
{
    ...
    }
    {
        ...
        /* `hash` is the image hash bootutil_img_hash computed above, and it IS
         * the founder leaf value -- passing it spares a second full-image pass. */
        FIH_CALL(pq_image_verify, fih_rc, pq_read_image, &fctx, it.tlv_end, hash,
